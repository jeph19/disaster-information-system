  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <Link href="/" className="flex items-center gap-3 no-underline" data-testid="link-brand">
        <div className="brand-mark">/\/</div><div><div className="brand-title">sentinel</div><div className="brand-subtitle">municipal EOC</div></div>
      </Link>
    ...
    <main className="main-area">
      <header className="topbar">
        ...
        <div><div className="topbar-kicker">Municipality of San Isidro / EOC-02</div><div className="topbar-title">Response coordination workspace</div></div>
...
function DashboardData({ summary, bars, healthOk }: { summary: DashboardSummary; bars: number[]; healthOk: boolean }) {
  ...
  return <><div className="metric-grid">
    <Metric label="Active incidents" value={summary.activeIncidents} foot={`${summary.totalIncidents} total in register`} tint="#f5e3c3" />
    <Metric label="People evacuated" value={summary.totalEvacuated} foot={`of ${number(summary.totalCapacity)} available capacity`} tint="#d6e8e0" />
    <Metric label="Evacuation centers" value={summary.totalEvacuationCenters} foot={`${number(summary.totalFamilies)} families registered`} tint="#dbe7ee" />
    <Metric label="Structures impacted" value={summary.partiallyDamaged + summary.totallyDamaged} foot={`${number(summary.totallyDamaged)} totally damaged`} tint="#f3d8d1" />
  </div><div className="dashboard-grid">
    <section className="panel"><div className="panel-header"><div><div className="panel-title">Active incident register</div><div className="panel-meta">{summary.activeIncidents} requiring coordination</div></div><Link href="/incidents" className="icon-btn" data-testid="link-dashboard-incidents"><ChevronRight size={16} /></Link></div>{summary.recentIncidents.length ? summary.recentIncidents.slice(0, 5).map(incident => <IncidentRow key={incident.id} incident={incident} />) : <EmptyPanel title="No recent incidents" copy="The desk is clear. New incidents will appear here as they are registered." />}</section>
    <section className="panel panel-body"><div className="chart-note"><div><div className="panel-title">Human impact by incident</div><div className="chart-sub">Affected population · current register</div></div><BarChart3 size={18} color="#367c74" /></div><div className="chart-value">{number(summary.populationByIncident.reduce((sum, item) => sum + item.population, 0))}<span className="chart-sub"> people</span></div><div className="sparkline" data-testid="chart-population">{bars.map((bar, i) => <div className="spark-bar" key={i} style={{ height: `${Math.max(10, (bar / max) * 100)}%` }} title={summary.populationByIncident[i]?.label ?? 'Incident'} />)}</div><div className="flex justify-between" style={{ color: '#8a9791', fontSize: 10, marginTop: 9 }}><span>Older</span><span>Most recent</span></div><div style={{ borderTop: '1px solid #e9e4d9', marginTop: 22, paddingTop: 17, display:'flex', gap:9, alignItems:'center' }}><Shield size={15} color="#c28b2b" /><span style={{ fontSize: 11, color:'#687775' }}>{healthOk ? 'All systems operational' : 'Checking service health'}</span></div></section>
  </div></>;
}
...
function IncidentDrawer({ value, onClose, onSave, pending }: { value: Incident | 'new'; onClose: () => void; onSave: (payload: IncidentInput, edit?: Incident) => void; pending: boolean }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={e => e.currentTarget === e.target && onClose()}><aside className="drawer"><div className="drawer-head"><div><div className="eyebrow">{edit ? 'Update record' : 'New record'}</div><div className="page-title" style={{ fontSize: 25, marginTop: 7 }}>{edit ? 'Edit incident' : 'Register incident'}</div></div><button className="icon-btn" onClick={onClose} data-testid="button-close-incident-drawer"><X size={18} /></button></div><form className="drawer-content" onSubmit={submit}><div className="form-grid">
    ...
    <Field label="Affected population"><input type="number" min="0" value={form.affectedPopulation} onChange={e => set('affectedPopulation', e.target.value)} data-testid="input-affected-population" /></Field>
    <Field label="Evacuated population"><input type="number" min="0" value={form.evacuatedPopulation} onChange={e => set('evacuatedPopulation', e.target.value)} data-testid="input-evacuated-population" /></Field>
...
function CenterDrawer({ incidentId, value, onClose, onSave, pending }: { incidentId: number; value: EvacuationCenter | 'new'; onClose: () => void; onSave: (payload: EvacuationCenterInput, edit?: EvacuationCenter) => void; pending: boolean }) {
  ...
  return <Drawer title={edit ? 'Edit evacuation center' : 'Add evacuation center'} eyebrow="Shelter register" onClose={onClose}><form onSubmit={submit}><div className="form-grid"><Field label="Center name" full><input required value={form.name} onChange={e => set('name',e.target.value)} data-testid="input-center-name" /></Field><Field label="Barangay"><input value={form.barangay} onChange={e => set('barangay',e.target.value)} data-testid="input-center-barangay" /></Field><Field label="Status"><select value={form.status} onChange={e => set('status',e.target.value)} data-testid="select-center-status">{Object.values(EvacuationCenterStatus).map(v => <option key={v}>{v}</option>)}</select></Field><Field label="Address" full><input value={form.address} onChange={e => set('address',e.target.value)} data-testid="input-center-address" /></Field><Field label="Capacity"><input type="number" min="1" value={form.capacity} onChange={e => set('capacity',e.target.value)} data-testid="input-center-capacity" /></Field><Field label="Current population"><input type="number" min="0" value={form.currentPopulation} onChange={e => set('currentPopulation',e.target.value)} data-testid="input-center-population" /></Field><Field label="Families"><input type="number" min="0" value={form.families} onChange={e => set('families',e.target.value)} data-testid="input-center-families" /></Field><Field label="Contact person"><input value={form.contactPerson} onChange={e => set('contactPerson',e.target.value)} data-testid="input-center-contact" /></Field><Field label="Contact number"><input value={form.contactNumber} onChange={e => set('contactNumber',e.target.value)} data-testid="input-center-number" /></Field><Field label="Men"><input type="number" min="0" value={form.men} onChange={e => set('men',e.target.value)} data-testid="input-center-men" /></Field><Field label="Women"><input type="number" min="0" value={form.women} onChange={e => set('women',e.target.value)} data-testid="input-center-women" /></Field><Field label="Children"><input type="number" min="0" value={form.children} onChange={e => set('children',e.target.value)} data-testid="input-center-children" /></Field><Field label="Seniors"><input type="number" min="0" value={form.seniors} onChange={e => set('seniors',e.target.value)} data-testid="input-center-seniors" /></Field><Field label="PWD"><input type="number" min="0" value={form.pwd} onChange={e => set('pwd',e.target.value)} data-testid="input-center-pwd" /></Field></div><FormActions onClose={onClose} pending={pending} saveLabel={edit ? 'Save center' : 'Add center'} testId="center" /></form></Drawer>;
}
...
function ReportsPage() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!selected) return; generate.mutate({ data: { incidentId:Number(selected), ...form } }, { onSuccess: setReport }); };
...
function ReportPreview({ report }: { report: SituationalReport }) {
  ...
  return <article className="report-paper" data-testid="report-preview"><div className="report-masthead"><div><div className="report-agency">MUNICIPAL EOC<br /><span style={{ fontSize:11, fontWeight:500 }}>SAN ISIDRO</span></div></div><div className="report-small">SITUATION REPORT<br />{report.reportNumber}<br />Generated {dateLabel(report.generatedAt)}</div></div><h2 className="report-title">{report.incident.title}</h2><div className="report-small">{report.incident.code} · {report.incident.location} · {report.incident.incidentType}</div><div className="report-section"><h4>Incident status</h4><table className="report-table"><tbody><tr><td>Status</td><td>{report.incident.status}</td></tr><tr><td>Severity</td><td>{report.incident.severity}</td></tr><tr><td>People affected</td><td>{number(report.incident.affectedPopulation)}</td></tr><tr><td>People evacuated</td><td>{number(report.incident.evacuatedPopulation || centerPopulation)}</td></tr><tr><td>Deaths / injuries / missing</td><td>{report.incident.deaths} / {report.incident.injuries} / {report.incident.missing}</td></tr></tbody></table></div><div className="report-section"><h4>Operational summary</h4><p>{report.operationalSummary}</p></div><div className="report-section"><h4>Priority needs</h4><p>{report.priorityNeeds}</p></div><div className="report-section"><h4>Actions taken</h4><p>{report.actionsTaken}</p></div><div className="report-section"><h4>Next steps</h4><p>{report.nextSteps}</p></div><div className="report-section"><h4>Resource and impact snapshot</h4><table className="report-table"><tbody><tr><td>Evacuation centers / sheltered</td><td>{report.centers.length} / {number(centerPopulation)}</td></tr><tr><td>Families registered</td><td>{number(report.centers.reduce((s,c) => s+c.families,0))}</td></tr><tr><td>Partial / total damage</td><td>{partial} / {total}</td></tr><tr><td>Prepared by</td><td>{report.preparedBy}</td></tr></tbody></table></div><div className="report-small" style={{ marginTop:32, paddingTop:12, borderTop:'1px solid #dedfd5' }}>Report date: {dateLabel(report.reportDate)} · Official operational record</div></article>;
}

function SettingsPage() {
  const health = useHealthCheck({ query: { queryKey: ['health-settings'] as const } });
  const [saved, setSaved] = useState(false);
  const [form, setForm] = useState({ municipality:'San Isidro', desk:'Municipal Emergency Operations Center', officer:'Mara Alvarez', email:'mara.alvarez@sanisidro.gov.ph', timezone:'Asia/Manila' });
  const save = (event: FormEvent) => { event.preventDefault(); setSaved(true); window.setTimeout(() => setSaved(false), 2600); };
